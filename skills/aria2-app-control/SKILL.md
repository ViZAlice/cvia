...
description: 通过本地 HTTP 控制通道操控 cvia 下载器桌面应用（Tauri，内核 aria2）：查看结构化页面快照、以 UI 同通道点击/填充、批量操作、添加和管理下载任务、Civitai 模型页识别与下载。当用户要求操作下载器、查下载进度、添加下载、清理任务或排查下载问题时使用。
---

# cvia 下载器控制技能

cvia（Windows，Tauri 应用，内核 aria2）内置**本地控制服务**，只监听 `127.0.0.1:33211`
